...
export function FeedPage() {
  ...
  const handleBuyOrReserve = async (item: Item) => {
    ...
    try {
      ...
      setItems((prevItems) =>
        prevItems.map((i) =>
          i.id === item.id ? { ...i, status: ItemStatus.RESERVED } : i,
        ),
      );

      const actionText =
        item.type === ItemType.DONATION ? "reservado" : "comprado (simulação)";
      setToastMessage({
    ...
    <div className={styles.page}>
      ...
      <header className={styles.header}>
        ...
        <p className={styles.subtitle}>
          Encontre materiais e equipamentos da comunidade acadêmica.
        </p>
      </header>
      ...
      <div className={styles.filters}>
        ...
        {(Object.entries(categoryLabels) as [ItemCategory, string][]).map(
          ([value, label]) => (
            <button
              key={value}
      ...
      {filteredItems.length === 0 ? (
        <div className={`${styles.empty} glass`}>
          ...
          <p>
            Não há itens disponíveis para este filtro no momento. Volte mais
            tarde ou tente outro filtro.
          </p>
        ...
        <Modal
          ...
          footer={
            <Button
              ...
              style={{
                opacity: mapItemToUI(selectedItem).isBuyDisabled ? 0.6 : 1,
                cursor: mapItemToUI(selectedItem).isBuyDisabled
                  ? "not-allowed"
                  : "pointer",
              }}
            >
